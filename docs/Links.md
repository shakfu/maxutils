# Useful Links

## Apple Developer Documentation

- [Notarizing macOS Software Before Distribution](https://developer.apple.com/documentation/security/notarizing_macos_software_before_distribution)

- [Hardened Runtime](https://developer.apple.com/documentation/security/hardened_runtime)

- [Notarizing Your FxPlug Plug-in](https://developer.apple.com/documentation/professional_video_applications/fxplug/notarizing_your_fxplug_plug-in/)

- [Technical Note TN2206 - macOS Code Signing In Depth](https://developer.apple.com/library/archive/technotes/tn2206/_index.html)

- [Customizing the Notarization Workflow](https://developer.apple.com/documentation/security/notarizing_macos_software_before_distribution/customizing_the_notarization_workflow) (This uses `notarytool` which is for xcode 13+)

- [Notarizing Apps when Developing with Xcode 12 and Earlier](https://developer.apple.com/documentation/security/notarizing_macos_software_before_distribution/customizing_the_notarization_workflow/notarizing_apps_when_developing_with_xcode_12_and_earlier) (This uses `altool` for xode 12 and earlier)

- [Customizing the Xcode Archive Process](https://developer.apple.com/documentation/security/notarizing_macos_software_before_distribution/customizing_the_notarization_workflow/customizing_the_xcode_archive_process)

## Apple Developer Focums

- [Signing a Mac Product For Distribution](https://developer.apple.com/forums/thread/128166)

- [stapling a notarization ticket](https://developer.apple.com/forums/thread/671514)

- [Code Signing a GUI python App](https://developer.apple.com/forums/thread/680719)

## Cycling 74 Forums

- [Max 8.1 - Mac OS 10.15 Catalina Support and Notarization](https://cycling74.com/articles/max-8-1-mac-os-10-15-catalina-support-and-notarization)

- [Mac standalone codesigning - 2021 update](https://cycling74.com/forums/mac-standalone-codesigning-2021-update)

## Stackoverflow / Stackexchange

- [Cannot codesign an app with Python and Node](https://stackoverflow.com/questions/54149144/cannot-codesign-an-app-with-python-and-node)

- [How do I create a DMG file from a directory?](https://apple.stackexchange.com/questions/48414/how-do-i-create-a-dmg-file-from-a-directory)

- [How to notarize a MacOS command line tool created outside of XCode?](https://stackoverflow.com/questions/64652704/how-to-notarize-an-macos-command-line-tool-created-outside-of-xcode)

- [How to code sign mac app on osx](https://stackoverflow.com/questions/26283847/how-to-code-sign-mac-app-on-osx)

- [Creating a .DMG](https://stackoverflow.com/questions/367751/creating-a-dmg)

## Github Tools

- [create-dmg: A shell script to build fancy DMGs](https://github.com/create-dmg/create-dmg)

- [dcm2niix-notarize.command](https://github.com/rordenlab/dcm2niix/blob/master/notarize.command)

- [macholib - used to analyze and edit Mach-O headers](https://github.com/ronaldoussoren/macholib/)

- [DiskImageDistribution - Tools to create and distribute macOS Applications through disk images](https://github.com/adib/DiskImageDistribution)

- [notary.sh](https://gist.github.com/lunixbochs/0ceeb23be5c3d5c6748165e61c6db493)

- [codesign - python scripts for signing, packaging, notarizing and stapling command line binaries](https://github.com/txoof/codesign)

- [munki-pkg - a simple tool for building packages in a consistent, repeatable manner from source files and scripts in a project directory.](https://github.com/munki/munki-pkg)

## Free Tools

- [SD Notary](https://latenightsw.com/sd-notary-notarizing-made-easy/)

## Commercial Toola

- [DropDMG](https://c-command.com/dropdmg/)

- [AppWrapper](https://ohanaware.com/appwrapper/)

## Comprehensive Guides

- [Codesigning and notarizing your standalone for distribution outside the Mac Appstore](https://lessons.livecode.com/m/4071/l/1122100-codesigning-and-notarizing-your-lc-standalone-for-distribution-outside-the-mac-appstore)

## Blog Articles

- [Notarizing Disk Images for Developer ID Distribution](https://indiespark.top/programming/notarize-disk-image-developer-id-distribution/)

- [How to Automate macOS Application Notarization](https://indiespark.top/software/automate-macos-application-notarization/)

- [Creating Packages from the Command Line - Tutorial and a Makefile - Part I](http://thegreyblog.blogspot.com/2014/06/os-x-creating-packages-from-command_2.html)

## Python Distribution

- [Freezing Your Code — The Hitchhiker's Guide to Python](https://docs.python-guide.org/shipping/freezing/)

- [Codesigning Python.framework](https://groups.google.com/g/munki-dev/c/FADUXPWJeds)
